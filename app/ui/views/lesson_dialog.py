...
class LessonDialog(ctk.CTkToplevel):
    def __init__(self, parent, class_subject_id, lesson_date, lesson_data=None, on_save=None):
        ...
        ctk.CTkLabel(self, text="Conteúdo:").grid(row=2, column=0, padx=10, pady=10, sticky="nw")
        self.content_text = ctk.CTkTextbox(self)
        self.content_text.grid(row=2, column=1, padx=10, pady=10, sticky="nsew")

        # Botões
        self.buttons_frame = ctk.CTkFrame(self)
        self.buttons_frame.grid(row=3, column=0, columnspan=2, padx=10, pady=10, sticky="ew")
    ...
    def save(self):
        ...
        try:
            if self.lesson_data:
                # Update
                data_service.update_lesson(self.lesson_data['id'], title, content, final_date)
            else:
                # Create
                data_service.create_lesson(self.class_subject_id, title, content, final_date)
